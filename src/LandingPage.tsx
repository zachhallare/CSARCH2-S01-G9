...
//
// Full-screen SVG rendered on a 1440×900 viewBox (xMidYMid slice fills any
// screen size). Traces are split into a left cluster (ARM/teal) and a right
// cluster (x86/orange), each mirrored for visual symmetry.
//
// Color rules (Guide.md):
//   ARM accent  #00C2D1   x86 accent  #FF8C42
//   Background  #0F1117   Panel       #1A1F2B
//
// All trace paths use orthogonal segments with 12 px 45° chamfers at corners,
// matching real PCB layout conventions. Via dots sit at every junction.
//
// Transitions are driven by inline CSS `transition` on the SVG <g> elements.
// `stroke`, `opacity`, and `filter` all animate at 0.5 s ease. Via circles use
// `fill: currentColor` so they inherit the animated group colour automatically.
...
function CircuitBoard({ hoveredArch }: { hoveredArch: HoveredArch }) {
  ...
  const tx: React.CSSProperties = {
    transition: "stroke 0.5s ease, opacity 0.5s ease, filter 0.5s ease, color 0.5s ease",
  };
  ...
  return (
    ...
    >
      ...
      {/*
          Each path is designed as real PCB traces:
          H/V segments → 45° chamfer via `L (x±C, y±C)` → next segment.
          All paths originate near the left edge and spread toward center-left,
          giving the impression the ARM chip is the source of these signals.
      */}
      <g
        fill="none"
        strokeWidth="1.5"
      ...
      >
        ...
        <path d={`M 0 540 H 112 L ${112+C} ${540+C} V 648 L ${112+2*C} ${648+C} H 295 L ${295+C} ${648-C} H 460`} />
      ...
      >
        ...
        <path d={`M 1440 540 H 1328 L ${1328-C} ${540+C} V 648 L ${1328-2*C} ${648+C} H 1145 L ${1145-C} ${648-C} H 980`} />
      ...
      {/* These ground the layout visually and make the full board feel connected. */}
      <g
        fill="none"
        strokeWidth="1"
        strokeLinecap="round"
