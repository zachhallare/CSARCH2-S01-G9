    ...
    <div className="w-full max-w-2xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.05)", backgroundColor: "#0F1117" }}>
...
function ChipButton({ label, accentColor, onClick }: { label: string; accentColor: string; onClick: () => void }) {
  ...
  const traceStyle: React.CSSProperties = hovered
    ? { strokeDasharray: "0.15 1", strokeDashoffset: "0", animation: "card-trace 2s linear infinite", opacity: 1 }
    : { strokeDasharray: "0.15 1", strokeDashoffset: "1", opacity: 0, transition: "opacity 0.3s ease" };
  ...
  return (
    ...
    >
      <svg className="absolute inset-0 w-full h-full rounded-xl overflow-visible pointer-events-none" style={{ zIndex: 2 }}>
        <rect
          x="0.75" y="0.75"
          ...
          strokeWidth="1.5"
          pathLength="1"
          style={{ filter: hovered ? `drop-shadow(0 0 3px ${accentColor}) drop-shadow(0 0 8px ${accentColor})` : "none", ...traceStyle }}
        />
      </svg>
        ...
        <div className="flex flex-col items-center justify-center h-full px-4 py-4 relative z-10">
          <div
            ...
            style={{
              ...
              boxShadow: hovered ? `0 0 4px 1px ${accentColor}, 0 0 12px 3px ${accentColor}` : "none",
          ...
          <span
            ...
            style={{ color: accentColor, opacity: hovered ? 1 : 0.85, transition: "opacity 0.25s ease" }}
          >
            {label}
          </span>
        </div>
...
function ProsConsItem({ text, icon, iconColor, accentColor }: { text: string; icon: string; iconColor: string; accentColor: string }) {
  ...
  const handleEnter = () => { setHovered(true); setPulseKey((k) => k + 1); };
  return (
    <li className="flex gap-2 rounded-lg p-3 relative" style={{
      backgroundColor: hovered ? `${accentColor}0D` : "rgba(15,17,23,0.4)",
      borderLeft: `2px solid ${hovered ? accentColor : "rgba(255,255,255,0.07)"}`,
      borderRight: "1px solid rgba(255,255,255,0.05)",
      borderTop: "1px solid rgba(255,255,255,0.05)",
      ...
      onMouseLeave={() => setHovered(false)}
    >
      <span key={pulseKey} className={`shrink-0 mt-0.5 inline-block ${hovered ? "animate-icon-pulse" : ""}`} style={{
        color: hovered ? "#ffffff" : iconColor,
        filter: hovered ? `drop-shadow(0 0 3px ${iconColor}) drop-shadow(0 0 8px ${iconColor}) drop-shadow(0 0 15px ${iconColor}AA)` : "none",
  ...
  switch (category) {
    ...
    case "useCases":
      return {
        ...
        back: (
          <ul className="space-y-2">
            {data.useCases.map((item, i) => <HoverItem key={i} accentColor={accentColor} className="p-3">{item}</HoverItem>)}
          </ul>
        ),
      };
    case "keyDevices":
